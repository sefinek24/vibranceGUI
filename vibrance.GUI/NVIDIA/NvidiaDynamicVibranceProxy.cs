...
{
    internal class NvidiaDynamicVibranceProxy : IVibranceProxy
    {
        ...
        public NvidiaDynamicVibranceProxy(List<ApplicationSetting> savedApplicationSettings, Dictionary<string, Tuple<ResolutionModeWrapper, List<ResolutionModeWrapper>>> currentWindowsResolutionSettings)
        {
            try
            {
                ...
                if (initializeLibrary()) InitializeProxy();
            ...
            catch (Exception ex)
            {
                ...
                var result = MessageBox.Show(NvapiErrorInitFailed, "vibranceGUI Error",
                    MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
                if (result == DialogResult.OK) Process.Start(GuideLink);
            }
        }
        ...
        {
            var gpuHandles = new int[NvapiMaxPhysicalGpus];
            var outputIds = new int[NvapiMaxPhysicalGpus];
            enumeratePhsyicalGPUs(gpuHandles);

            foreach (var gpuHandle in gpuHandles)
                if (gpuHandle != 0)
                {
                    var systemType = getGpuSystemType(gpuHandle);
                    if (systemType == NvSystemType.NvSystemTypeUnknown)
                    {
                        MessageBox.Show(NvapiErrorSystypeUnknown, "vibranceGUI Error",
        ...
        private static void OnWinEventHook(object sender, WinEventHookEventArgs e)
        {
            if (_applicationSettings.Count > 0)
            {
                ...
                {
                    var displayHandle = GetApplicationDisplayHandle(e.Handle);
                ...
                else
                {
                    ...
                    //test if a resolution change is needed
                    var currentScreen = Screen.FromHandle(processHandle);
                    if (_vibranceInfo.neverChangeResolution == false &&
                        _gameScreen != null &&
                    ...
                    {
                        if (_gameScreen != null && !_gameScreen.DeviceName.Equals(currentScreen.DeviceName)) return;
        ...
        private static int GetApplicationDisplayHandle(IntPtr hWnd)
        {
            if (hWnd != IntPtr.Zero)
            {
                var primaryScreen = Screen.FromHandle(hWnd);
                if (primaryScreen != null)
                {
                    var deviceName = primaryScreen.DeviceName;
                    var handle = GCHandle.Alloc(deviceName, GCHandleType.Pinned);
                    var id = getAssociatedNvidiaDisplayHandle(deviceName, deviceName.Length);
                    handle.Free();
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto)]
        private static extern bool unloadLibrary();
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto)]
        private static extern int getActiveOutputs([In] [Out] int[] gpuHandles, [In] [Out] int[] outputIds);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto)]
        private static extern void enumeratePhsyicalGPUs([In] [Out] int[] gpuHandles);
        ...
        private static extern bool getGpuName([In] [Out] int[] gpuHandles, StringBuilder szName);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Ansi)]
        private static extern bool getDVCInfo(ref NvDisplayDvcInfo info, int defaultHandle);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto)]
        private static extern int enumerateNvidiaDisplayHandle(int index);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto)]
        private static extern bool setDVCLevel([In] int defaultHandle, [In] int level);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto)]
        private static extern bool isWindowActive(ref IntPtr hwnd);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Ansi)]
        private static extern bool isCsgoStarted(ref IntPtr hwnd);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto)]
        private static extern bool equalsDVCLevel([In] int defaultHandle, [In] int level);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto)]
        private static extern NvSystemType getGpuSystemType(int gpuHandle);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Ansi)]
        private static extern int getAssociatedNvidiaDisplayHandle(string deviceName, [In] int length);
